...
html+='<section class="card"><details class="warp-acc" '+(customEps?'open':'')+'><summary>'+esc(t('warp.detail.custom_eps_title'))+'</summary><p class="field__hint">'+esc(t('warp.detail.custom_eps_desc'))+'</p><div class="field"><textarea class="input textarea textarea--mono" id="warp-custom-eps" rows="5" dir="ltr" spellcheck="false" placeholder="162.159.192.1:2408">'+esc(customEps)+'</textarea></div><div class="btn-row"><button type="button" class="btn btn--primary btn--sm" data-action="warp-custom-eps-save" data-id="'+esc(a.id)+'">'+esc(t('common.save'))+'</button></div></details></section>';
const ov=a.amnezia_overrides||S.warp.amnezia||{};
const AMZK=[['Jc','warp.amnezia.jc'],['Jmin','warp.amnezia.jmin'],['Jmax','warp.amnezia.jmax'],['S1','warp.amnezia.s1'],['S2','warp.amnezia.s2'],['S3','warp.amnezia.s3'],['S4','warp.amnezia.s4'],['H1','warp.amnezia.h1'],['H2','warp.amnezia.h2'],['H3','warp.amnezia.h3'],['H4','warp.amnezia.h4']];
...
AMZK.forEach(function(kv){const key=kv[0];const valA=ov&&ov[key]!==undefined&&ov[key]!==null?ov[key]:'';amh+='<div class="field"><label class="field__label" for="amza-'+key+'">'+esc(t(kv[1]))+'</label><input class="input" type="text" id="amza-'+key+'" dir="ltr" value="'+esc(String(valA))+'"></div>'});
...
html+='<section class="card"><details class="warp-acc" '+(a.amnezia_overrides?'open':'')+'><summary>'+esc(t('warp.amnezia.title'))+'</summary>'+amh+'</details></section>';
