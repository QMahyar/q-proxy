...
The VLESS subscription surface is exactly four targets — `base64`, `singbox`, `clash`, and `xray` — plus the browser info page. There is no pipeline for adding further VLESS targets in this codebase state:

1. **Types:** `SubFormat` in `src/core/ua.ts` is `"base64" | "singbox" | "clash" | "xray"`; `classifyUA` sniffs sing-box tokens, then clash tokens (`clash`/`mihomo`/`stash`), then `xray-core`, then base64-client tokens, browsers, and defaults to base64.
2. **Emitters:** `src/nodes/emitters/registry.ts` (`EMITTERS` covers sing-box, clash, and xray; base64 renders in `src/subscription/render.ts`).
...
- `GET api/status`; `POST api/killswitch {enabled}` → `{killSwitch, rev}`; `GET api/suburls` (four targets + info entry)
