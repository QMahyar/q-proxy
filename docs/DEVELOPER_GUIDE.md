...
| Storage | KV (`QPROXY_KV`) for settings/WARP/throttle state + D1 (`QPROXY_DB`, `migrations/`) for write-hot state | `wrangler.toml`, `src/types/env.ts`, `src/settings/store.ts:bootstrapD1` |
