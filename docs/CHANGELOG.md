...
### Added
- Clash/Mihomo YAML subscriptions end to end: pure `emitClashYaml` emitter (block-style proxies, `skip-cert-verify`, `url-test`/`select` PROXY group, user block/bypass rules, `MATCH,PROXY`), `?target=clash` + `clash`/`mihomo`/`stash` UA sniffing (after sing-box tokens), content-type/extension/labels/drift parity, per-format hub rows, wizard and Telegram surfaces.
- Xray-core JSON subscriptions end to end: pure `emitXrayJson` emitter (sniffing socks inbound, DoH DNS, vless+ws outbounds, direct/block rails, `leastPing` balancer over exactly the served nodes with direct fallback when empty), `?target=xray` + `xray-core`-only UA sniffing (`foxray` explicitly stays base64), same drift parity.
- Foreign-subscription import: `POST /{sp}/api/sub-import` (session+CSRF) returns a parse-only per-source preview (`vless://` + base64 + bare `host:port`; other schemes reported per line; never fetches, never stores) and the hub Import card confirms the merge into `customEndpoints` via the guarded settings save.
- Best-ping guidance block in the hub naming the auto-selecting profiles per client (base64 honestly excluded).
- Deploy Button way (one-click, signup inside the flow) with KV/D1 binding follow-ups; five-minute quickstart with a proof table; deployment matrix; cut-justification note.
- Optimistic settings concurrency: reads expose `rev`, `PUT settings/save` accepts `baseRev` and answers `409 CONFLICT` instead of silently overwriting; the panel tracks, sends, and rebases with a conflict toast.
- Optional `cdnHost`/`cdnSni` front override for non-worker addresses (CDN masking triple); workerless-rescue packaging (downloaded WARP configs work offline).
- Dashboard observability (`[observability]`, full head sampling); `GET /healthz` answers static `{ok:true}`.

### Fixed
- Telegram bot identity locked to numeric chat IDs (`@username` rejected at validation, fail-closed at match, legacy values migrated).
- Server password floor: min 8 + letter + digit on setup and change (422 field error), matching the clients.
- `deploy.py` secret hygiene: `--password-stdin`/`QPROXY_PASSWORD`/hidden prompts, no printed passwords, hidden token paste.
- Pool/webhook inputs stream-cut at their caps (256 KiB / 64 KiB) instead of buffered whole; tunnel parks no post-response lifetime.
- Pool empty states carry Retry; WARP preset switches confirm while accounts exist; protocol saves refresh hub links; wizard lists all formats.
- Usage honesty: every surface reads one source labeled `estimated`, with header/display agreement pinned by tests.

### Changed
- Subscription surface is four targets (`base64`/`singbox`/`clash`/`xray`); `GET api/suburls` returns four URLs + info entry.
- `ExecutionContext` threaded explicitly; module-global context holders deleted.
- `Env` generated by `wrangler types`; `nodejs_compat` enabled; dead cron removed.
- Delivery: dist 366077 → 380028 bytes (~357 → ~374 KiB); full suite 1008 → 1073+ tests green.
