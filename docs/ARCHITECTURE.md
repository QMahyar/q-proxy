...
> **Rev 2026-09-19 glowup-19 release packaging:** v1.6.0 tagged. Route-table drift check: the only table change this release is row 20c (`POST api/sub-import`, added with its router-spec matrix entries in ticket 16); §2.2/§2.5/§3 bodies otherwise verbatim. Format inventory settled at four sub targets + four WARP families; doc-code counts corrected everywhere (guide pipeline, suburls rows, README, AGENTS). Bundle 366077 → 383702 B (+4.8%); suite 1008 → 1073 tests, typecheck clean. Deferred to the release environment (no backend/browser/clients here): 12-step visual walk green twice with zero console errors, live Deploy Button click, fresh-person quickstart trial, real-client connects (Clash/Xray import + merge result). Frozen body text left verbatim by design.

> **Rev 2026-09-19 glowup-18 secondary builds (golden: NEW xray, additive):** all three ticket-06 approvals shipped. (1) `?target=xray`: new pure emitter `src/nodes/emitters/xray-json.ts` (socks inbound with sniffing, DoH DNS, vless+ws outbounds, direct/block rails, `leastPing` balancer over exactly the served nodes, empty profile falls back to direct; `flow` carried, ECH omitted — no certain Xray schema key); `SubFormat` + `SUB_FORMATS` + content-type/extension + labels + drift parity follow; UA sniffing resolves `xray-core` only (`foxray` explicitly stays base64). Golden flag: NEW goldens pinned, no old→new move. (2) Custom-CDN triple: optional `cdnHost`/`cdnSni` settings (empty = auto) overriding Host/SNI on non-worker-hostname addresses; full settings-pattern agreement (interface + defaults + descriptors + FL rows + bilingual dict + validate); existing goldens byte-identical unset. (3) Workerless rescue as docs + one panel sentence (downloaded WARP confs work offline; no generator). Rejected items (deeper bot, workerless generator) untouched — no telegram/warp diff in this batch. Frozen §2.5/§3 body text left verbatim by design (no new routes).

> **Rev 2026-09-19 glowup-17 best-ping guidance:** auto-selection is a property of the profile formats, not the hub — sing-box and Clash emit `url-test`/`select` PROXY groups whose members are exactly the served nodes (membership pinned by goldens), so any client following the hub guidance lands on a live node. The hub carries a bilingual guidance block naming that per client (base64 honestly excluded: static lists cannot auto-select). No new routes, formats, or goldens moved. Frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-16 foreign-subscription import:** new `POST /{sp}/api/sub-import` (row 20c; session+CSRF) returns a parse-only per-source preview (`{sources: [{tag, endpoints, invalid}]}`) and stores nothing — no URL fetching, so no SSRF surface. Pure parser `src/subscription/foreign.ts` (vless:// + base64 + bare host:port; other schemes reported per line, never throws). Confirm reuses `PUT settings/save` (CF port families + 64-line cap enforced, baseRev conflicts honored), so merged endpoints serve in every format through the existing generator. Hub Import card with preview/confirm actions; walk covers preview → confirm → served → revert. Frozen §2 body text left verbatim; §3 gains row 20c per the route-table rule.

> **Rev 2026-09-19 glowup-15 newcomer docs:** `docs/QUICKSTART.md` (button → setup card → forced change → wizard → first connected client, with per-client tabs and a proof table mapping every claim to code/tests), a deployment matrix in `docs/DEPLOYMENT.md`, and a cut-justification note (`USER_GUIDE.md` §19 — each absence points at its decision). Stale doc-code drift fixed alongside: wrangler snippet gains `compatibility_flags` + `[observability]`, settings GET example gains `rev`, README drops the pre-cut protocol list and rotting counts. Client tabs are drift-guarded (`test/docs/quickstart.spec.ts` pins `?target=` values to `SUB_FORMATS`). Screenshots stay placeholder-marked for the walk environment (no browser here). Frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-14 subscriptions hub P1:** the hub is data-driven from the served `subUrls` (base64+singbox+clash rows with expanders, download links, mode switch, and copy/QR — no per-format hardcoding, so served formats can never drift from rows). Protocol saves now refresh shown links like general/addresses saves (stale-read fix). Per-user token cards stay out: their backend died in spec-001 (dead token URLs serve camouflage), so a per-user row would promise an unshipped capability. Frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-13 observability + save safety:** `[observability]` enabled with head sampling 1 (field names verified against the wrangler config schema). Settings saves are optimistic-concurrency guarded: reads expose `rev` (GET settings, bootstrap), `PUT settings/save` accepts optional `baseRev` and answers `409 CONFLICT` (new `ConflictError`) instead of silently overwriting on mismatch; absent `baseRev` keeps legacy behavior for non-panel writers. The panel tracks `S.rev` from boot, sends it on section + endpoint saves, and rebases (fresh fetch, re-render, conflict toast) on 409. Residual window (documented, not fixed): KV edge-cache reads can lag ~60s, so a conflict inside that window still overwrites — a D1-backed revision counter is the follow-up. Failed background flushes log instead of vanishing (pinned by test). Frozen §2/§3/§6 body text left verbatim by design.

> **Rev 2026-09-19 glowup-12 buffering caps + tunnel lifetime:** attacker-controlled inputs are stream-cut at their caps, never buffered whole — `fetchPoolUrl` reads the pool response through a 256 KiB cancelling reader (fail-closed `[]` past the cap); the Telegram webhook parses via `readJsonObject` (64 KiB cap, content-length fast path; over-cap stays `silentOk`). Tunnel lifetime holds no post-response parking (`waitUntil` lives only in the bounded counter-flush and audit sinks — pinned by guard test). Frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-11 WARP hub P1:** endpoint presets card shows a live Custom count chip (`warp.endpoints.count`) and confirms before switching the global selection while accounts exist (`confirmDialog` with new bilingual `warp.confirm.endpoints_title/body`; unchanged selections and account-less saves skip the dialog). Restores the confirm-before-touch-data behavior lost in the spec-002 checkbox rebuild. Families + Amnezia toggle, URLs-first detail with collapsed token, and error-with-retry loads already held; counts already read the 4-family registry. Frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-10 one-click + guided setup:** Deploy Button way added (`docs/DEPLOYMENT.md` Way B + README badge; signup inside the button flow, KV/D1 bindings as explicit dashboard follow-ups). In-panel wizard step 2 now lists per-format subscription URLs with copy+QR (was base64-only — Clash users stalled) using the shared home-teaser helpers; wizard s1/s2 copy corrected EN+FA (VLESS-only reality, per-client URLs). Stall points from the (unanswered) first-install questionnaire are tracked in `.scratch/glowup-v16/issues/10-stall-tracker.md` with owners. Frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-09 pool health in panel:** both pool surfaces (home card, settings egress card) implement the state triad — `loadingBox` → rows / `emptyCard` with retry / `errorCard` with retry. Empty states previously rendered a bare hint with no retry; both now use `emptyCard({title:'egress.pool.empty',cta:'common.retry'})` wired to the existing retry hooks (`data-retry="home-pool"`, `data-action="pool-fetch"`). Guard test pins the pairing; the visual walk asserts pool settle states (rows / error+retry / empty+retry) on home and egress. Frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-08 honest usage accounting:** every usage surface reads one source (`readUsage`) and is labeled an estimate — `usageView` (`src/handlers/api/status.ts`) serializes `{requestsToday, requestsTotal, bytesUpTotal, bytesDownTotal, estimated: true}` for `/api/status` and `/api/bootstrap`; `estimatedDownloadBytes` (`src/subscription/headers.ts`) is the single download formula behind both `Subscription-Userinfo` and the Telegram `/status` line (human units + estimate marker, EN+FA); the panel status card carries a bilingual estimates qualifier. No server-side Cloudflare token is stored (rejected tradeoff — see gap 12), so platform GraphQL usage stays out by design; the header/display mismatch is closed by construction, not docs. Frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-07 Clash emitter (golden: NEW, additive):** `SubFormat` is `base64|singbox|clash`; new pure emitter `src/nodes/emitters/clash-yaml.ts` (`emitClashYaml`, Mihomo-compatible block-style YAML: vless+ws proxies with `skip-cert-verify`, `url-test`/`select` PROXY group, user block/bypass rules ahead of RFC1918 DIRECT rails, `MATCH,PROXY` final; `flow` omitted — vision flows don't apply over WS; `DEFAULT_PROXY_DNS` hoisted to the registry for both emitters). UA sniffing resolves `clash|mihomo|stash` after the sing-box tokens (composite sing-box+clash UAs stay sing-box); `SUB_FORMATS` + content-type (`text/yaml`) + extension (`yaml`) + info-page labels + `buildSubUrls` + panel `FORMAT_LABELS`/`FORMAT_ORDER` + bilingual `fmt.clash`/`fmt.clash.hint` follow. Golden flag: NEW goldens pinned (`test/nodes/emitters/clash-yaml.spec.ts`) — no old→new byte move (the spec-001-deleted clash emitter is not resurrected; this is a new Mihomo-profile writer). Frozen §2.5/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-05 deploy secrets + health hygiene:** `deploy.py` credential intake never exposes secrets — panel password via `--password-stdin` (piped first line) > `QPROXY_PASSWORD` > deprecated `--password` (process-list warning) > hidden `getpass` prompt, with fail-fast strength checks on every non-interactive path; API token via `CLOUDFLARE_API_TOKEN` or a hidden `getpass` paste (`--token` kept working with a warning); the deploy summary no longer prints the password. `GET /healthz` answers static `{ok:true}` (`no-store` kept; version/colo dropped). Frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-04 server password rules:** setup and password-change enforce the client-demanded floor via shared `passwordStrengthError` (`src/auth/password.ts`): min 8 chars with a letter and a digit, else `ValidationError({newPassword})` (422 field error the panel renders inline). Panel `security.rule` (EN+FA) now states the letter-and-digit floor. The login-page setup meter stays stricter (uppercase) — a stricter client over a uniform server floor. Frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-03 telegram identity lock:** bot identity is numeric chat IDs only. `telegram.chatId` validation accepts `""` or `-?\d{1,20}` and rejects `@usernames` with a field error naming the numeric requirement; the webhook `chatMatches` compares `String(chat.id)` exactly and returns `false` for any stored `@` value (fail closed — username-presenting updates disclose nothing and toggle nothing); `migrateSettings` clears a legacy `@` chatId to `""` on load so pre-lock stores keep passing validation; panel `advanced.tg.hint` (EN+FA) documents the numeric-only rule with the @userinfobot pointer. Frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-02 explicit request context:** `ExecutionContext` is threaded explicitly (`routeRequest(req, env, ctx)`, `RouteHandler` 4th param `ctx?`, `afterResponse(ctx, p)`, `recordConnection/recordBytes(env, bytes?, ctx?)`, `audit(action, detail, env?, ctx?)`); the module-global holders (`buffer.ctx`/`bindCounterContext`/`getCounterContext`, `auditCtx`/`bindAuditContext`) are deleted. The tunnel no longer parks the whole WS session in `waitUntil` (the 101 response keeps the isolate alive); `driveSession` carries `ctx` only for its `recordBytes` flushes. `ctx` params are optional so existing 3-arg test call sites still compile; the router always passes the real context. Debug-flag globals kept (plain booleans, not request/context/env holders — observability follow-up). Proven by `test/core/request-context.spec.ts` (per-request `waitUntil` isolation for flushes and audit inserts, plus a no-globals assertion). Frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-19 glowup-01 runtime scaffolding:** `wrangler.toml` gains `compatibility_flags = ["nodejs_compat"]` (prod now resolves what the worker test project resolves); `Env` is generated by `wrangler types` into `src/env.d.ts` (global, from the wrangler.toml bindings) replacing hand-written `src/types/env.ts` (deleted with its 18 import sites — `satisfies ExportedHandler<Env>` on the worker default export keeps binding renames a build error); the dead `[triggers]` cron plus the `scheduled` handler are removed (the body ran only boot init, already covered on every fetch; a real sweep is future work). Frozen §2.1/§8.3 body text left verbatim by design. Rerun `wrangler types` after any wrangler.toml change — CI enforces a clean diff.

> **Rev 2026-09-17 spec-003 WARP output consolidation (clarified: toggle matrix, global switch, camouflage refusal):** WARP served surface cut 17→4 families in `src/warp/formats/registry.ts` (`wireguard-conf` ZIP, `singbox` JSON, `v2rayn` base64 links, `throne` always-Amnezia links); `-amnezia` twins collapsed into emit-time toggle branches (`conf.ts` single zip honoring `ctx.amneziaEnabled`, `singbox.ts` single entry, `emitThrone` always-Amnezia), `emitWireguardUri` kept as an internal helper with its route deleted, `src/warp/formats/proxies.ts` (clash/surge/surfboard/loon/egern) plus `emitXray` and the legacy sing-box branch deleted. New `WarpGlobalSettings.amneziaEnabled` (default `false`, same KV record, accepted alongside values by the existing `amnezia` PUT which purges served copies; GET exposes it); `expandAccount` resolves the toggle into `WarpEmitContext` and ignores per-account `amnezia_overrides` at render, while create/update paths ignore override writes (stored values left inert, never migrated). Deleted format names fall through to `handleCamouflage` identical to unknown URLs (the distinct 404 is gone); invalid tokens keep existing handling. Panel: WARP groups/detail trimmed to the four families, the per-format Amnezia show-toggle plus per-account Amnezia card/handlers deleted, one global switch with a re-download hint in the Amnezia card (new EN+FA `warp.amnezia.toggle`/`toggle_hint` and `warp.groups.v2rayn`; 22 orphaned keys deleted; stale Amnezia descriptions rewritten), `ui-walk.mjs` step 6 re-pointed at four-groups-plus-toggle (step count kept). Survivor plain goldens stay byte-identical with the toggle OFF; new toggle-ON goldens equal the retired twin bytes. No new top-level routes; frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-17 spec-002 preset+custom endpoints (clarified: replace-model, uniform variants, global WARP source):** §2.2 Settings loses `addresses`/`AddressSetting` (per-address host/SNI/label/country/enabled gone) and gains `cdnPresets: string[]` (default `[]`, all shipped presets opt-in), `customEndpoints: string[]` (default `[]`, verbatim valid lines), `warpPresets: string[]` (default `["default"]`), `warpCustomEndpoints: string[]` (default `[]`); `SETTINGS_VERSION` stays 3 (a bump would reclassify v3 backups as pre-cut) with no new migration step — pre-cut `addresses` data drops via deep-merge, not migrated. New descriptors + `endpointLinesField` validation in `src/settings/validate.ts` (VLESS boxes enforce CF port families with whole-save rejects naming `line N`, 64-line cap; WARP box allows any port 1–65535). New data module `src/nodes/cdn-presets.ts` (8 entries, Principle-IV exception header, CI-pinned CF ports). `collectAddresses` rebuilt in `src/nodes/generate.ts` around a shared `resolveEndpointLines` helper (also consumed by `address-probe.ts`): enabled presets + parsed custom lines + worker-hostname fallback, deduped by lowercased `host:port`; `?country=` filter and `resolveHostname` list use deleted. Uniform variants: presets get fragment nodes like any TLS address. WARP `expandAccount` renders from the global selection (empty → `default` preset fallback, unknown ids tolerated); per-account `endpoint_list` retired in `src/handlers/api/warp.ts` (create/update paths ignore it; preset DELETE guards the global selection instead) with stored values left inert. Panel: endpoint section rebuilt (preset checklist widget + custom textarea + probe card), per-account WARP endpoint UI replaced by one global card, EN+FA dicts (`endpoints.*`, `warp.endpoints.*`, orphaned `addresses.*`/`country`/preset-switch keys deleted), `?country=` hub docs removed. No new top-level routes; frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-16 spec-001 VLESS+WARP slim-down (constitution v1.2.0 triage):** §2.2 Settings schema cut to the VLESS+WARP single-admin surface and `SETTINGS_VERSION` bumped 2→3. Removed field families (descriptors + validate branches + defaults go together): `vmessEnabled/trojanEnabled/ssEnabled`, `vmessUuid/trojanPassword/ssPassword/ssMethod/ssDirect`, `vmessPath/trojanPath/ssPath`, `proxyIpMode`, `nat64Prefixes`, `chainProxy` (+`ChainProxySettings`), `remoteDns`, `urlTestIntervalSec`, `remoteNodes` (+`RemoteNodeKind/RemoteRealitySetting/RemoteHy2Setting/RemoteNodeSetting/MAX_REMOTE_NODES`), `remoteSubUrls`, `speedtestIntercept`, `totp` (+`TotpSettings`), `camouflage.url`; `CamouflageSettings` is `{mode}` with `CamouflageMode = "off" | "static"`; `SENSITIVE_SETTING_PATHS` drops `totp`. New `MIGRATIONS[2]` strips the removed keys and maps `camouflage.proxy→static` (v1 step kept). Import rule: backup `version < 3` is rejected whole with a pre-cut `ValidationError` (nothing applied, nothing merged); export emits surviving fields with secrets stripped. Boot rule: first boot on a v<3 store purges `qproxy:users` + `qproxy:user-usage/total/activity:*` KV keys and drops D1 tables `users/user_totals/user_usage/user_activity` (new `migrations/0002_drop_users.sql`), writes one removal log line, stamps v3 (idempotent); counters/audit survive. `PRESERVED_FIELDS` trimmed to the surviving identity set. §2.4 `ProxyNode` is `VlessNode` only (VMess/Trojan/SS/Reality/Hy2 interfaces, `isRemoteNode`, `REMOTE_NODE_KINDS` deleted); `generateNodes`/`share-uri`/`naming` VLESS-only. §2.5 `EMITTERS` is `{singbox}` (clash/surge/loon/quantumult/`yaml-writer` deleted), `SUB_FORMATS` is `base64|singbox` (deleted `?target=` values are 400 invalid, sniffing never resolves them), base64 renders own nodes only (`merge.ts` deleted), sing-box `proxy-dns`/`urltest` interval baked in (`EmitOptions` drops `remoteDns`/`urlTestIntervalSec`). §3 route deletions: `users-sub`, `my-ip`, `api/users`, `api/version` (handlers + `ApiRouteName`/`SecureRoute` rows); removed tunnel paths, dead token URLs and deleted API routes fall through to static camouflage identical to unknown paths (`my-ip`/`user-sub` contract rows gone). Tunnel: `tunnel/chain/`, `nat64.ts`, `speedtest.ts` deleted; `EgressVia` is `direct|proxyip` (direct-first + pool tail, `dialTcp` moved to `egress.ts`); per-IP `ratelimit.ts` moved to `src/tunnel/`; `TunnelKind` is `"vless"`; handler is VLESS-only with settings-driven early data. Auth: TOTP removed (password-only login, no pre-auth cookie/recovery codes); Telegram trimmed to status/sub-links/kill on-off + rewritten help (usage/expiry/sweep deleted, keyboard is Status/Subscription/Kill ON/Kill OFF); scheduled worker only runs `ensureInitialized`. Panel: `users.js`/`users-modal.js`/`totp.js` deleted (19-part `PANEL_JS_ORDER`), removed cards/fields pruned, `FORMAT_LABELS`/`FORMAT_ORDER` base64+singbox, unknown/deleted views redirect home, dict 564→392 keys EN+FA (new `general.backup.precut` for pre-cut import errors; boot purge note stays server-log English per the validation-message gap), `ui-walk.mjs` steps re-pointed at surviving surfaces (12 steps kept). Tests: 97→73 spec files; moved goldens (singbox single-VLESS profile, base64 own-nodes-only, UA/negotiate deleted-format refusals, egress direct-first, camouflage equivalence, router removed-route matrix, bytes-spec VLESS reject/1011-fail/camouflage cases) plus new `test/workers/settings-cut.spec.ts` (export round-trip, pre-cut reject, boot purge). Rationale recorded in ADR-010; golden moves pre-authorized by the same triage. No new top-level routes; frozen §2/§3 body text left verbatim by design.

> **Rev 2026-09-09 ui-task19 amendments (write-only fields + derived headers):** §2.2 Settings schema loses three write-only fields — `localDns` (validated but consumed by no emitter/handler/tunnel path), `sourceUrls` (the former Sources-subtab list, consumed by nothing; the Egress pool resolves from `proxyIps`/`proxyIpPoolUrl`/DoH only) and `AddressSetting.city` (collected but never used in generation; naming uses `country` only) — descriptor rows, validate branches and defaults removed together; `SETTINGS_VERSION` unchanged (2) and old KV blobs migrate safely because `deepMergeDefaults` only copies keys present in `DEFAULT_SETTINGS` (absent keys drop silently on next save) and `validateSettings` rebuilds from defaults (unknown patch keys never read). Panel drops the Egress source-URLs card, the Advanced local-DNS field and the address-editor city input with their dict families (`egress.remoteSubs.*`, `advanced.localDns.label`, `addresses.field.city`). `throttleHeaders(updateIntervalHours, now)` (arg previously optional-and-ignored) now derives `Profile-Update-Interval` from the setting via `profileUpdateInterval()` — clamped to the validated 1–168 h range (NaN ⇒ 12, the default) — replacing the hardcoded `"60"` (minutes) that contradicted `subUpdateIntervalHours`; the 60 s `Cache-Control`/`Expires` throttle triple is intentionally unchanged (per-fetch edge throttle, unrelated to client update cadence; `versionedEdgeCacheKey` already carries the settings etag). `subscriptionHeaders` forwards `meta.updateIntervalHours`; WARP subs pass `s.subUpdateIntervalHours`. Deliberately moved goldens (headers spec): base64 golden `Profile-Update-Interval` `"60"`→`"12"` (default setting), interval-pin test now asserts the 1 h clamp, `throttleHeaders` golden calls pass the setting explicitly; handler specs (subscribe/users-sub/warp-sub) assert `"12"` (fixtures use the 12 h default). Per-user activity loses the zero-writer `bytesUp`/`bytesDown` plumbing: `UserActivityDay`/`UserActivityDelta` shrink to `{day, requests}`/`{requests?}` (KV activity blobs with legacy byte fields still parse — extra keys ignored), D1 `user_activity` writes/reads drop the `bytes_up`/`bytes_down` columns (existing deployed tables keep the columns via `CREATE TABLE IF NOT EXISTS`; fresh installs match `migrations/0001_init.sql`), and the KV→D1 migration copies only `requests`; `GET /api/users/:id/activity` keeps its shape and window rules, global `counters` bytes (tunnel-fed, consumed by `Subscription-Userinfo`) are untouched. AGENTS.md field count corrected 66→73. No new top-level routes; frozen §2/§3 body text left verbatim by design.
...
> **Rev 2026-09-04 onboarding-v2 + TOTP-UX amendments:** §2.2 Settings gains `passwordIsBootstrap: boolean` (default `false`, descriptor `{kind:"bool"}`) and `seededAt: number` (default `0`, descriptor `{kind:"int", min:0, max:4_102_444_800_000}` — 2100-01-01 ceiling); no `SETTINGS_VERSION` bump (deep-merge defaults fill absent keys, migrate-safe). `fillIdentity` stamps `seededAt = Date.now()` only when it was `0` — a first-seed timestamp, never refreshed. `validateSettings` gains `validateBootstrapConsistency`: `passwordIsBootstrap: true` with `passwordHash === null` fails validation (a bootstrap password must already exist). §3 row 14 setup: after the existing TOCTOU `loadSettingsFresh` re-check (409 `ALREADY_SET`), a seeded-but-passwordless panel whose `seededAt` is older than 24 h answers `409 SETUP_WINDOW_EXPIRED` (`seededAt > 0` guard keeps pre-rev blobs setup-open). Frozen API JSON contract, `POST api/auth/login`: full-auth success data becomes `{hasPassword:true, mustChangePassword:true}` while `passwordIsBootstrap` is set, `{hasPassword:true}` otherwise (frozen §3 row left verbatim); the pre-auth TOTP pending shape `{totpRequired:true}` is unchanged. Router: `ApiRouteDescriptor` gains an optional `bootstrap: "allow" | "read"` exemption column; `dispatchApi` wraps every session-authed handler in `bootstrapGated`, which while `passwordIsBootstrap` answers `403 PASSWORD_CHANGE_REQUIRED` ("choose a personal admin password before using the panel") except the allowlist rows `auth-logout` (`allow`), `auth-password` (`allow`), `bootstrap` (`allow`) and `settings-get` (`read` ⇒ GET allowed, PUT blocked) — auth:none routes (login/setup/logout) bypass the gate. `POST /{sp}/api/auth/password` (row 14b) saves `passwordIsBootstrap: false` with the new hash, lifting the gate (session-floor bump unchanged). Deploy-direct handoff contract (flagged by the wave, lands in a follow-up `scripts/deploy-direct.mjs` commit): the script generates the first panel password in the `qproxy-XXXXXXXX` form, prints it exactly once, polls the freshly seeded worker until the worker responds, and exits 1 when the password handoff fails. Panel TOTP enable flow (panel sources only, en+fa dicts): the setup view is the single place the 10 recovery codes are ever shown — copy-all plus a plain-text download (`q-proxy-recovery-codes.txt`) — and `Verify and enable` stays disabled until the explicit "I saved my recovery codes" toggle is on, with a defense-in-depth re-check inside the `totp-confirm` action; the TOTP card tracks `started`/`confirmed` state so running setup again while a flow is in progress (or previously started) requires a confirm dialog and surfaces the new-secret-invalidates-old-QR warning in the idle and setup views; after enable the codes view is replaced by the persistent device-clock-skew warning (`totp.warn.clock`), and the login page's rejected-code hint points at the device clock or a recovery code (`login.totp.rejected`). No new top-level routes; frozen §2/§3 body text left verbatim by design.
> **Rev 2026-09-04 wave6 amendments:** §2.1 `Env` gains `QPROXY_DB: D1Database` (required binding); §8.3 wrangler shape gains a `[[d1_databases]]` stanza (binding `QPROXY_DB`, `database_name = "q-proxy"`, `migrations_dir = "migrations"`). §5 storage split: write-hot state moves to D1 tables `users` / `user_totals` / `user_usage` / `user_activity` / `counters` / `audit_log` / `meta` (schema in `migrations/0001_init.sql`, mirrored by exported `D1_SCHEMA` in `src/users/store.ts`); the settings blob, WARP store, login-throttle / session-floor / ratelimit keys stay on KV. New `StoreEnv` type (`QPROXY_KV` + optional `QPROXY_DB`) threads both stores; boot `ensureInitialized` calls `bootstrapD1` (`ensureD1Schema` + idempotent KV→D1 copy guarded by `meta` row `kv_migrated_v1`, legacy KV keys deleted after copy). Quota/activity/counter increments are single-statement `ON CONFLICT` UPSERTs (race-free, replacing the KV read-modify-write approximations); counters read/flush D1-first with KV fallback on error (`COUNTERS_KV_KEY` export). `audit(action, detail, env?)` keeps the JSON log line and additionally persists to `audit_log` via `waitUntil` (`bindAuditContext`, wired through `bindCounterContext`). §2.5 sing-box DNS schema: default output is typed servers — `proxy-dns` `{type, server, detour: "PROXY"}` plus `local-dns` `{type: "local"}`, lone-server form `{type, server}` with no detour, zero `"address"` keys; domain-hosted upstreams gain `domain_resolver: "local-dns"` (never self-referential), non-default ports emit `server_port`, non-default https/h3 paths emit `path` (scheme map https/http→https:443, tls→tls:853, tcp/udp→53, quic→853, h3→443). §2.10 panel source layout: dev sources live in `src/ui/panel/` (`shell.html` + 3 markers `<!--panel:head-js-->/<!--panel:css-->/<!--panel:js-->`, `head.js`, `app.css`, 9 JS parts concatenated in fixed order dict→lib→qr→home→warp→users→chrome→settings→actions) assembled by `scripts/build-single-file.mjs` (marker splice, `node --check` over script blocks, rewrite-only-on-change); `src/ui/panel.html` is git-kept generated output. §7 vitest: `unit` excludes `test/d1/**`, `workers` includes `test/d1/**/*.spec.ts` with `d1Databases: ["QPROXY_DB"]`; `test/helpers/seed.ts` gains `applyD1Schema`/`resetD1`/`seedLegacyKvForMigration`/`clearLegacyKvForMigration`. No new top-level routes; frozen §2/§3/§5 body text left verbatim by design.
> **Rev 2026-09-04 wave5b amendments:** Admin-owned external remote nodes (ADR-008 model): new ProxyNode kinds `reality`/`hy2` (+ `isRemoteNode`/`REMOTE_NODE_KINDS`), new `remoteNodes[]` setting (cap 20, per-kind validation incl. 32-byte pbk decode and sid hex rules, SSRF host-literal guard, no private-key field by construction), `buildVlessRealityUri`/`buildHy2Uri` grammars, clash (`reality-opts`+flow, hysteria2) and sing-box (`reality`+flow+tcp, hysteria2) emission; surge/loon/quantumult omit remote kinds. Rendered into admin subscriptions only — `users-sub` strips remote kinds before protocol filtering (even `protocols=all`). Caveats: hy2 share-URI follows the de-facto grammar (no formal spec); hy2 outbounds emit `skip-cert-verify`/`tls.insecure` for the self-signed-VPS common case (documented in panel help).
> **Rev 2026-09-04 wave5a amendments:** §2.4 `VlessNode` gains optional `flow?: string | null`, `SSNode` gains optional `direct?: boolean` (absent/null/false ⇒ legacy output byte-identical). §2.2 Settings schema gains `vlessFlow: string` (default `""`, descriptor `{kind:"enum", allowed:["", "xtls-rprx-vision"]}` + VLESS-section panel select with en/fa dicts `protocols.flow.*`) and `ssDirect: boolean` (default `false`, descriptor `{kind:"bool"}` + SS-section panel toggle with en/fa dicts `protocols.ssDirect.*`); no `SETTINGS_VERSION` bump (deep-merge defaults fill absent). `generateNodes` stamps the configured flow onto TLS VLESS nodes only (plain nodes stay `flow: null`) and marks every SS node `direct` when enabled. `buildVlessShareUri` appends `flow=` after the transport params when set; `buildSSShareUri` emits a plain `ss://` URI with no plugin segment when direct. Clash emits `flow` on VLESS proxies and drops `plugin`/`plugin-opts`/`skip-cert-verify` on direct SS proxies; sing-box emits `flow` on VLESS outbounds and drops `plugin`/`plugin_opts` on direct SS outbounds (surge/loon untouched). §2.7 vision bodyCodec semantics: `createVlessInbound` negotiates `xtls-rprx-vision` by substring-matching the handshake addons field (protobuf-framed or raw JSON) on TCP commands only — UDP keeps the `LengthPacketReader` codec even when vision is requested; handshake-time framing coalesces complete initial-payload frames and seeds the trailing partial frame into the codec; the vision codec frames bodies as `u16be length + payload` (64 KiB buffer cap; overflow resets and returns null, split frames buffer across `decodeUp` calls, downlink encoder has a null header and maps empty/oversize encodes to empty; parsers never throw). Transport spikes land as decisions only (no route-table change; §Scope "No inbound gRPC/xhttp/REALITY" stays true): ADR-006 gRPC DEFER (no response-trailer/h2-stream API in the fetch handler — trailer-less "gRPC" rejected, grpc-web rejected for zero client interop), ADR-007 XHTTP DEFER (nearest-term candidate; gated on pinning Xray `transport/internet/xhttp` wire semantics plus a production-edge full-duplex probe — short-POST subset first, split up/down over KV rendezvous rejected), ADR-008 REALITY termination DEFERRED permanently (no inbound TCP/handshake control behind the edge) with a specified-but-unapproved remote-reference model (S effort; admin-owned external nodes in admin subs only, never `users-sub`, private key never in KV), sequenced by ADR-009 (XHTTP probes → REALITY-remote product call → gRPC parked on platform support). No new top-level routes; frozen §2/§3 body text left verbatim by design.
> **Rev 2026-09-04 wave4 amendments:** §2.3 `SubFormat` gains `"quantumult"` (UA tokens `quantumult`/`quanx`, sniffed after surge, before loon); new pure emitter `emitQuantumultConf` in `src/nodes/emitters/quantumult-conf.ts` (`[general]`/`[server_local]`/`[policy]`/`[filter_local]` sections, single static PROXY group with `direct` fallback; vless/trojan TLS-only, plain vmess/ss kept with ws obfs, vmess `auto` cipher mapped to `none`, fragment nodes hidden unless `isFragment`, IPv6 bracketed) registered in `EMITTERS` (now five sync formats) and wired through one-liners in `SUB_FORMATS` (`src/subscription/negotiate.ts`), `SUB_CONTENT_TYPES` (`src/subscription/render.ts`, `text/plain`), `EXTENSIONS` (`src/subscription/headers.ts`, `.conf`), `FORMAT_LABELS` (`src/handlers/subscribe.ts`, "Quantumult X") and `buildSubUrls` (`src/handlers/api/status.ts`, `?target=quantumult`); `GET api/suburls` now returns 7 entries. §2.2 `AddressSetting` gains optional `country?: string` / `city?: string` (no `SETTINGS_VERSION` bump; deep-merge defaults fill absent); save-time validation uppercases/trims `country` (must be 2 letters, empty = absent) and trims `city` (64-char cap, empty = absent); `generateNodes` parses `?country=` from `NodeBuilderContext.request` (comma-separated, case-insensitive, invalid tokens ignored, missing/empty = no filter) and drops tagged addresses not in the list while always keeping untagged entries and the worker-hostname fallback — `/sub/u/{token}` honors `?country=` through the same path. §3 row 22b gains `POST /{sp}/api/users/bulk` (session+CSRF; body `{ids: string[1..50], patch: {enabled?, expiresAt?} (>=1 field) | {delete: true} alone}`; response `{updated, deleted, unknown}` with unknown ids skipped and tokens never returned; bulk never creates users so the ≤50 invariant holds). Non-contract panel batch (`src/ui/panel.html` only, en+fa dicts): token-regen show-once modal with copy+QR, global shortcuts (Ctrl/Cmd+S apply, Ctrl/Cmd+K search-or-home, g-h home, Ctrl/Cmd+Z / Shift+Ctrl/Cmd+Z per-section undo/redo, `?` cheatsheet), ≤500px mobile pass, `qp_traffic` localStorage SVG chart on Home (empty state until history accumulates), 30-day backup-nudge banner, Users-tab bulk bar, per-address country/city fields. No new top-level routes; frozen §2/§3 body text left verbatim by design.
> **Rev 2026-09-04 wave3b amendments:** §2.2 Settings schema gains `allowedIps: string[]` (default `[]` = allow all; descriptor `{kind:"custom"}` + Security-section panel binding with en/fa dicts `security.allowlist.label/hint`); save-time validation trims/dedupes/drops empties, caps at 64 entries, accepts exact IPv4/IPv6 literals or v4/v6 CIDR (prefix `<=32`/`<=128`) and rejects hostnames/ports/malformed prefixes via `fields.allowedIps`. New pure matcher `isIpAllowlisted(ip, list)` in `src/auth/guard.ts` (empty list ⇒ true; exact match case-insensitive and bracket-tolerant; CIDR via existing `cidrContains`, family-strict; malformed entries fail closed); `requireAuth` verifies the session first (missing/bad ⇒ 401) then the client IP from `CF-Connecting-IP` via existing `clientIp` (non-listed ⇒ 403 `ForbiddenError`); login/setup routes are `auth:none` so they stay reachable and lockout is never permanent. New `audit(action, detail)` in `src/core/log.ts` emitting a JSON `info` line (`scope:"audit"`, existing `t/level/scope/message/extra` envelope) via the `log.info` seam — allowlist design serializing only caller-supplied fields, so secrets appear only if a caller passes them (no call site passes values, only key names/ids/booleans). Instrumented actions: `settings.save|reset|import` with `{ip, keys}` (sorted changed top-level keys, values never logged), `killswitch` with `{ip, enabled}`, `warp.account.create|import|update|delete|regenerate-token` and `warp.preset.create|update|delete` with `{ip, id}`, `warp.amnezia.update` with `{ip}`. Per-user activity aggregates: new KV keys `qproxy:user-activity:<yyyy-mm-dd>:<hash>` (never plaintext tokens) holding `{day, requests, bytesUp, bytesDown}` (`UserActivityDay`; `recordUserActivity`/`getUserActivity` in `src/users/store.ts`, `USER_ACTIVITY_DEFAULT_DAYS=7`/`MAX_DAYS=31`, 1–31 clamp, chronological, zeros for missing/corrupt days, migrate-safe optional byte fields); deltas buffer in-isolate and flush every 32 ops/60 s (mirrors totals batching; hot path adds zero extra KV writes), exported `flushPendingUserActivity`, activity rows migrated on token regen; new admin shape `GET /api/users/:id/activity?days=` ⇒ `{activity: UserActivityDay[]}` (unknown/malformed id ⇒ 404, non-GET ⇒ 404, response never contains token/hash). Per-user connection rate limiting: new `src/users/ratelimit.ts` token-bucket keyed by token-hash string (`RATELIMIT_WINDOW_MS=60000`, `RATELIMIT_MAX_CONNECTIONS=30`, `RATELIMIT_BURST=10`, `RATELIMIT_PREFIX="qproxy:ratelimit:"`, `RATELIMIT_TTL_SECONDS=120`; pure `ratelimitKey(hash,now)` ⇒ `qproxy:ratelimit:<hash>:<window>` and `consumeBucket(prev,now)`; KV-backed `tryConsume` fail-open on KV errors); `src/tunnel/relay.ts` gains optional `RelayOptions.gate: RelayGate` (`RelayGateDecision = boolean | {allowed, retryAfterMs?}`), consulted once at relay start with throw ⇒ allow, deny ⇒ `finish(1008)` — admission seam only in this wave, no per-user tunnel wiring yet. Telegram inline keyboards: new exported `telegramMenuKeyboard()` (3×2 `inline_keyboard`, `callback_data` namespace `tg:status|usage|sub|expiry|kill-on|kill-off`); `/start` and `/menu` reply with help text + keyboard (other commands unchanged, no keyboard); webhook routes `callback_query` through the unchanged secret auth (path/`X-Telegram-Bot-Api-Secret-Token`) and chat gate (callback message chat, `from` as fallback), answering `answerCallbackQuery` and editing the message in place via `editMessageText` (fallback `sendMessage` when no message id); `setWebhook allowed_updates` extended to `["message","callback_query"]`. No new top-level routes; no `SETTINGS_VERSION` bump (deep-merge defaults fill `allowedIps`).
> **Rev 2026-09-04 wave1 amendments:** Surge emitter gains VLESS + Shadowsocks lines (Surge 5 grammar; supersedes the 2026-08-24 vmess+trojan-only note) and Loon emitter gains Shadowsocks + `tls-profile` fingerprint + `ech=` lines; VLESS/Trojan share-URI `type=ws` confirmed pre-existing with regression specs. Clash TLS proxies gain `skip-cert-verify: true`. `resolveHostname` now implements hostnameOverride ?? primaryCustomDomain ?? request-hostname. SSRF: camouflage.url/chainProxy.uri/proxyIps rejected at save for private targets, camouflage redirect hops re-checked per hop, private proxyIP candidates filtered from failover. Oversize early-data closes the tunnel with 1009 (was silent drop). VMess chunk counter terminates before 16-bit wrap instead of reusing nonces. Login throttle is KV-backed (new KV keys `qproxy:login-fail:<sha256-ip>:<minute-window>`, 120s TTL, fail-open on KV error). Subscription edge-cache keys carry `_v=<settings etag>` so settings changes bust stale entries (60s throttle headers unchanged). Remote-sub fetch parallelized under a 10s total budget (1MB/response + 1MB total caps kept); relay gains a 2MB downlink hard cap closing with 1009. Telegram `@username` chat match is case-insensitive.
> **Rev 2026-09-04 wave2b amendments:** Password hashing is peppered — PBKDF2 input is HMAC-SHA256(key=sessionSecret, message=password), so KV-theft alone no longer enables offline brute-force. Verify order peppered-100k → unpeppered-100k → unpeppered-15k-legacy; any legacy success transparently rehashes to peppered-100k on login (setup/password-change hash with the fresh secret). Session `jti` removed (was generated but never verified; revocation remains solely the `iat` vs `qproxy:min-iat` floor) — payload is exactly `{exp, iat}` and pre-upgrade cookies carrying `jti` still verify (verifier ignores unknown fields).
...
└── src\
    ...
    ├── settings\
    │   ├── store.ts                  # loadSettings/saveSettings/ensureInitialized/invalidateSettingsCache (60s TTL cache)
...
| 20c | POST | `/{sp}/api/sub-import` | `handlers/api/sub-import.ts` | session+CSRF | parse-only foreign-list preview → `{sources}`; stores nothing (Rev glowup-16) |
...
```
 Panel SPA (F)                             Worker (E)
   ...
   │                                          │   stepwise MIGRATIONS if version drift → merge DEFAULTS → cache(60s TTL)
```
